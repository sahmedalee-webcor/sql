create or replace view SCDATAANALYTICS_PROD.DWH.VIEW_FCT_TCC_GVA(
	SPRH_BC_REQUESTNO,
...
WITH SPRH AS(
    ...
    from DWH.FCT_PRINPUT_HEADER_GVA PRH
        LEFT JOIN DWH.DIM_CUSTOMERS_BC CBC ON 
        PRH.CUSTOMER_NUMBER =CBC.NO AND UPPER(PRH.SOURCE)=UPPER(CBC.SOURCE)
        AND CBC.CURRENT_FLAG=1 AND UPPER(CBC.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) 
    WHERE PRH.current_flag = 1
        AND UPPER(PRH.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --2642 ROWS --2618 rows
),
SPRL AS (
    SELECT 
   ...
   --SELECT * 
   FROM DWH.FCT_PRINPUT_LINE_GVA PRL
    WHERE current_flag = 1
        AND UPPER(PRL.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
...
VENDBC_FILTERED AS (
    ...
    FROM (
        ...
        FROM DWH.DIM_Vendors_BC
        WHERE CURRENT_FLAG = 1
          AND UPPER(SOURCE) IN (
              SELECT SOURCE
              FROM VIEW_REPORT_FILTERS_GVA
              WHERE TCC_FLAG = 1
          )
    ) t
    WHERE rn = 1
...
CSHR AS(
    SELECT DISTINCT         
        ...
        BC_CS_SYSTEM_CREATED_BY,
        BC_CS_SYSTEM_MODIFIED_BY
    FROM DWH.FCT_COSTINGSHEET_HEADER_GVA CSH --3272    
        -- LEFT JOIN DWH.DIM_Vendors_BC VENDBC ON CSH.VENDOR_CODE = VENDBC.NO
        LEFT JOIN VENDBC_FILTERED VENDBC ON CSH.VENDOR_CODE = VENDBC.NO
        AND UPPER(CSH.SOURCE) = UPPER(VENDBC.SOURCE)
        AND VENDBC.CURRENT_FLAG = 1
        AND UPPER(VENDBC.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
        ...
        AND UPPER(PT.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
        ...
        AND UPPER(CUSTBC.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
    WHERE CSH.current_flag = 1
        AND UPPER(CSH.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
...
CSL AS (
    ...
    FROM DWH.FCT_COSTINGSHEET_LINE_GVA CSL
    WHERE CURRENT_FLAG = 1
        AND UPPER(CSL.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
        ...
        AND EXISTS (
            ...
            FROM (
                    ...
                    SELECT CASE WHEN VT.BC_Customer_PO_Number IN ('','TBA','PENDING','**','....') THEN trim(PS.SI_EXTERNAL_DOCUMENT_NUMBER)  ELSE trim(VT.BC_Customer_PO_Number) END AS CUSTOMER_PO_NUMBER,
                        ...
                        BL_NUMBER
                    FROM DWH.FCT_VESSEL_TRACKING_GVA VT
                    ...
                    AND PS.CURRENT_FLAG = 1 AND UPPER(PS.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
                    WHERE VT.CURRENT_FLAG = 1
                        AND BC_CUSTOMER_PO_NUMBER IS NOT NULL AND VT.SOURCE IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
                ) B
...
              CFF.SL_PTP_DISCHARGEPORTATA,
            ...
            FROM DWH.FCT_VESSEL_TRACKING_GVA VTBC
                    ...
                    AND UPPER(PS.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
                    LEFT JOIN CFF
                    ON VTBC.BC_Customer_PO_Number = TRIM(CFF.ST_NAME)
            where --BOOKING_REFERENCE_NUMBER <> '' 
                VTBC.current_flag = 1
                AND UPPER(VTBC.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) 
...
,
        PO_H_BC AS (
            ...
            WHERE POH.CURRENT_FLAG = 1
                AND UPPER(POH.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --8110 -- 8157 rows
        ),
        PO_L_BC AS(
            ...
            WHERE CURRENT_FLAG = 1
                AND UPPER(SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --8620
        ),
        PO AS (
            ...
            FROM PO_H_BC
                ...
                AND PT.CURRENT_FLAG = 1
                AND UPPER(PT.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --6530
            WHERE PO_H_BC.POH_CURRENT_FLAG = 1
                AND UPPER(POH_SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
...
,
        POSTED_INVOICE_H AS(
            ...
            WHERE PPH.CURRENT_FLAG = 1
                AND UPPER(PPH.SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --4647
        ),
        POSTED_INVOICE_L AS (
            SELECT 
            ...
            WHERE CURRENT_FLAG = 1
                AND UPPER(SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --8049
        ),
...
,
        SO_H_BC AS(
            ...
            WHERE CURRENT_FLAG = 1
                AND UPPER(SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --3391
        ),
        SO_L_BC AS(
            ...
            WHERE CURRENT_FLAG = 1
                AND UPPER(SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1) --13291
        ),
        SO AS (
            SELECT *
            FROM SO_H_BC
...
,
        POSTED_SALES_H AS (
            ...
            WHERE CURRENT_FLAG = 1
                AND UPPER(SOURCE) IN (SELECT SOURCE FROM VIEW_REPORT_FILTERS_GVA WHERE TCC_FLAG =1)
        ),
        POSTED_SALES_L AS (
            SELECT 
                BC_SI_DOC_NUMBER,
...
--SELECT * FROM POSTED_SALES;
        ...
        SELECT 
           ...
           --A.SPRL_ITEM_DESCRIPTION,
            ...
            CASE WHEN UPPER(A.CSH_SOURCE)= 'BUSINESS_CENTRAL' THEN 'DMCC' 
                ...
                ELSE UPPER(A.CSH_SOURCE) END AS SOURCING_OFFICE,  -- TBD
            PPH_VENDOR_NAME,
            POH_VENDOR_NAME,
            PPI_L_FRT_PI_LINE_AMOUNT_LCY,
            PPI_L_INS_PI_LINE_AMOUNT_LCY,
 ...
 --, SPRL_LINE_NUMBER 
    ...
    SELECT 
        ...
        --Sales Invoice Value,
        CASE WHEN CSH_costingsheet_date < '2025-07-1' 
        OR (FINAL_VT_GRN_DATE IS NOT NULL OR FINAL_VT_GRN_DATE NOT IN ('1900-01-01','0001-01-01')) THEN 'No'
        WHEN CSH_costingsheet_date>= '2025-07-1' AND ( FINAL_VT_GRN_DATE IS NULL  OR FINAL_VT_GRN_DATE IN ('1900-01-01','0001-01-01')) THEN 'Yes'
...
, TCC AS (
    ...
    SELECT * FROM CUST WHERE (CSH_CS_STAGE <>'Cancelled' ) --3585 rows
